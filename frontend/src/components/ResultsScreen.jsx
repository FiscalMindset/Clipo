  ...
  return <article className="result-clip-card">
    ...
    <div className="result-card-copy">
      ...
      <div className="result-card-actions">
        <button onClick={() => onPlay(clip)}>Preview</button>
        <button onClick={() => onStudio(clip)} style={{ background: 'linear-gradient(120deg, #7c3aed, #a855f7)', color: 'white', border: 'none' }}>Studio</button>
        <a href={getDownloadUrl(jobId, clip.filename)} download>{icon('download')} Download</a>
      </div>
...
export default function ResultsScreen({ jobId, onReset, onNavigate }) {
  ...
  const handleStudioSave = async (options) => {
    ...
    try {
      // In a full implementation, you'd pass options.position too if the API supports it
      const result = await generateCaptions(jobId, studioClip.id, options.preset);
      captionClip(studioClip.id, result);
      setStudioClip(null);
    } catch (err) {
      alert(err.message || 'Could not generate captions.');
    }
  };
  ...
  return <div className="results-shell"><div className="results-aura" /><div className="results-frame">
    ...
    <section className="results-summary"><div><strong>{clips.length}</strong><span>Clips</span></div><div><strong>{formatDuration(totalRuntime)}</strong><span>Total runtime</span></div><div><strong>Completed</strong><span>Processing status</span></div><div><strong>Clipo-selected</strong><span>Clip quality</span></div></section>
    ...
    {visibleClips.length ? <section className="results-grid">{visibleClips.map((clip) => <ClipCard key={clip.id} clip={clip} jobId={jobId} onPlay={setPlayingClip} onRename={renameClip} onDelete={removeClip} onStudio={setStudioClip} />)}</section> : <section className="results-empty"><div>{icon('spark')}</div><h2>Generate your first clips.</h2><p>{clips.length ? 'No clips match those filters.' : 'Your best Clipo moments will appear here once processing finishes.'}</p>{clips.length ? <button onClick={() => { setQuery(''); setDurationFilter('all'); }}>Clear filters</button> : <button onClick={onReset}>Start a new job</button>}</section>}
  </div>
  {playingClip && <VideoPlayer clip={playingClip} clips={clips} jobId={jobId} onSelect={setPlayingClip} onClose={() => setPlayingClip(null)} onRename={renameClip} onDelete={(id) => { removeClip(id); setPlayingClip(null); }} onCaption={captionClip} />}
  {studioClip && <CaptionStudio clip={studioClip} onClose={() => setStudioClip(null)} onSave={handleStudioSave} />}
  </div>;
}
